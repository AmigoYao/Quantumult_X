...
if (body) {
    switch (!0) {
        ...
        case /^https:\/\/app\.bilibili\.com\/x\/v2\/feed\/index\?/.test($request.url):
            try {
                ...
                for (let c of a.data.items)
                    ...
                    else if (!c.hasOwnProperty("ad_info") && -1 === c.card_goto ? .indexOf("ad") && ["small_cover_v2", "large_cover_v1", "large_cover_single_v9"].includes(c.card_type)) b.push(c);
                else continue;
        ...
        case /^https?:\/\/app\.bilibili\.com\/x\/resource\/show\/tab/.test($request.url):
            try {
                const a = new Set([177, 178, 181, 102, 104, 106, 486, 488, 489]);
        ...
        case /^https?:\/\/api\.live\.bilibili\.com\/xlive\/app-room\/v1\/index\/getInfoByRoom/.test($request.url):
            try {
                let a = JSON.parse(body);
                a.data.activity_banner_info = null, a.data ? .shopping_info && (a.data.shopping_info = {
                    is_show: 0
                }), a.data ? .new_tab_info ? .outer_list && a.data.new_tab_info.outer_list.length && (a.data.new_tab_info.outer_list = a.data.new_tab_info.outer_list.filter(a => 33 != a.biz_id)), body = JSON.stringify(a)
            } catch (a) {
        ...
        case /^https:\/\/app\.bilibili\.com\/x\/v2\/splash\/list/.test($request.url):
            try {
                ...
                if (a.data && a.data.list)
                    for (let b of a.data.list) b.duration = 0, b.begin_time = 2240150400, b.end_time = 2240150400;
                body = JSON.stringify(a)
            } catch (a) {
